...
{
	int i,j,k,l,m;
	double L[101][6][11],O[101],L_d[101][6][11],s,err,alpha=1;
	srand(time(NULL));
	...
	{	
		...
		{
			err=err+fabs(L[j][h+1][1]-O[j]);   	
			L_d[j][h+1][1]=(L[j][h+1][1]-O[j])*d_S(L[j][h+1][1]); //forming L_delta of last layer
		}
		for(j=h;j>=1;j--)	//layer
		{
			for(k=1;k<=n;k++)	//entry
		...
		for(j=0;j<=h+1;j++)	//layer
		{
			for(k=1;k<=nl[j-1];k++)	//updating weights
			{
				for(l=1;l<=nl[j];l++)
				{
					...
					for(m=1;m<=n;m++)
					{
						s=s+(L[m][j-1][k]*L_d[m][j][l]);
					}
					w[j][k][l]=w[j][k][l]-(alpha*s);
				}
			}
		}
