...
int main()
{
	...
	int G[n][n], X[n2], Y[n2], pattern[n2], step_size[n2];
	char text[n2+100];
	...
	while(true)
	{
		...
		if(p_filled > 0 && p_filled < 100)
		{
			...
			p_complete *= 100;
			p_complete = trunc(p_complete*pow(10,6))/pow(10,6);
		}
		else
			p_complete = p_filled;
		...
		sprintf(text,"percentage completed = %.6f%% ",p_complete);
		outtextxy(0,0,text);
		sprintf(text,"percentage filled = %.6f%% ",p_filled);
		...
		p_filled *= 100.0/n2;
		p_filled = trunc(p_filled*pow(10,6))/pow(10,6);
			
		for(i=0;i<n;i++)
		{
			for(j=0;j<n;j++)
			{
				setfillstyle(1,G[i][j]);
				bar(700+i*s,350+j*s,700+(1+i)*s,350+(1+j)*s);
				if (N%2 == 0)
				{
					bar(700+i*s,350-j*s,700+(1+i)*s,350-(1+j)*s);
					bar(700-i*s,350+j*s,700-(1+i)*s,350+(1+j)*s);
				...
				{
					bar(700+i*s,350-(j-1)*s,700+(1+i)*s,350-j*s);
					bar(700-(i-1)*s,350+j*s,700-i*s,350+(1+j)*s);
					bar(700-(i-1)*s,350-(j-1)*s,700-i*s,350-j*s);
				}
			}
		}
	}
